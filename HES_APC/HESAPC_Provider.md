...
| npi | NULL |  |  |
| dea | NULL |  |  |
| specialty_concept_id | tretspef,mainspef | Specialty_concept_id will be mapped to SNOMED Concept_id by using tretspef if it is not equal to ‘&’ OR mainspef if it is not equal to ‘&’ to retrieve the target_concept_id from source_to_concept_map where source_to_concept_map WHERE source_code = tretspef <> ‘&’ OR mainspef <> ‘&’ AND source_vocabulary_id = “HES_SPEC_STCM”".|  |
| care_site_id |NULL | | |
| year_of_birth | NULL |  |  |
